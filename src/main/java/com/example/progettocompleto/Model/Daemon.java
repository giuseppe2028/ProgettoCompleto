...
public class Daemon {
    ...
    public static List<Object> getDatiProfiloImpiegato(int matricola) {
        List<Object> ritorno = new ArrayList<>();
        String sql = " SELECT matricola,nome,cognome,cf,data_nascita,indirizzo_residenza,ruolo,mail,iban,recapito_telefonico,mail_personale,sesso, ref_servizio FROM Amministrativo WHERE matricola = ? UNION SELECT matricola,nome,cognome,cf,data_nascita,indirizzo_residenza,ruolo,mail,iban,recapito_telefonico,mail_personale,sesso, ref_servizio FROM Impiegato WHERE matricola = ?";
        try (PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
            preparedStatement.setInt(1, matricola);
            preparedStatement.setInt(2, matricola);
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery();) {
                while (resultSet.next()) {
                    ...
                    ritorno.add(resultSet.getString("mail_personale"));
                    ritorno.add(resultSet.getString("sesso"));
                    ritorno.add(resultSet.getInt("ref_servizio"));
    ...
    public static boolean updateDatiProfilo(long recapito, String indirizzo, String iban, String mail, InputStream path, int matricola) {


        try {

            String query = "UPDATE Impiegato SET recapito_telefonico=?,indirizzo_residenza=?, IBAN=?  ,mail_personale=?,foto_profilo=? WHERE matricola=?";
            PreparedStatement pstm1 = conn.prepareStatement(query);
            pstm1.setLong(1, recapito);
            pstm1.setString(2, indirizzo);
            pstm1.setString(3, iban);
            pstm1.setString(4, mail);
            pstm1.setBlob(5, path);
            pstm1.setInt(6, matricola);
            pstm1.execute();
            String query1 = "UPDATE Datore I SET indirizzo_residenza=?,recapito_telefonico=?, IBAN=?  ,mail_personale=?,foto_profilo=? WHERE matricola=?";
            PreparedStatement pstm2 = conn.prepareStatement(query1);
            pstm2.setString(1, indirizzo);
            pstm2.setLong(2, recapito);
            pstm2.setString(3, iban);
            pstm2.setString(4, mail);
            pstm2.setBlob(5, path);
            pstm2.setInt(6, matricola);
            pstm2.execute();
            String query2 = "UPDATE Amministrativo I SET indirizzo_residenza=?,recapito_telefonico=?, IBAN=?  ,mail_personale=?,foto_profilo=? WHERE matricola=?";
            PreparedStatement pstm3 = conn.prepareStatement(query2);
            pstm3.setString(1, indirizzo);
            pstm3.setLong(2, recapito);
            pstm3.setString(3, iban);
            pstm3.setString(4, mail);
            pstm3.setBlob(5, path);
            pstm3.setInt(6, matricola);
            pstm3.execute();
            return true;
    ...
    public static boolean updateFoto( InputStream path, int matricola) {
        try {
            String query1 = "UPDATE Datore  SET foto_profilo=? WHERE matricola=?";
            PreparedStatement pstm2 = conn.prepareStatement(query1);
            pstm2.setBinaryStream(1,path);
            pstm2.setInt(2, matricola);
            pstm2.execute();
            return true;
...
//TODO aggiungere la union con l'amministrativo
   ...
   /* public static Date getInizioturno(int matricola, Date date) {
    ...
    public static List<PropostaTurno> getPropostaTurni() {
        ...
        try {
            String sql = "Select tipo_turno,cognome, I.ref_servizio,I.ruolo,data_turno,nome,inizio_turno,fine_turno,ref_impiegato from PropostaTurno PT,Impiegato I where PT.ref_impiegato = I.matricola";
            PreparedStatement pstm = conn.prepareStatement(sql);
            resultSet = pstm.executeQuery();

            while (resultSet.next()) {
                listaRitorno.add(new PropostaTurno(resultSet.getString(1), resultSet.getString(2) + " " +resultSet.getString(6), resultSet.getInt(3), resultSet.getString(4), resultSet.getDate(5).toLocalDate(),resultSet.getTime(7).toLocalTime(),resultSet.getTime(8).toLocalTime(),resultSet.getInt(9)));
            }
            return listaRitorno;
        ...
        public static void rimuoviPeriodo(int id){
            try {
...
try{
 ...
 public static void updatePassword(String nuovaPass,int matricola){
    ...
    public static Stipendio getResocontoStipendio(int matricola, String mese, int anno){
        ...
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            if(resultSet.next()){
                return new Stipendio(resultSet.getInt(1),resultSet.getString(2),resultSet.getInt(3),resultSet.getInt(4),resultSet.getInt(5),resultSet.getDouble(6),resultSet.getDouble(7),resultSet.getInt(8),resultSet.getInt(9),resultSet.getBoolean(10),resultSet.getDouble(11),resultSet.getDouble(12),resultSet.getDouble(13),resultSet.getInt(14),resultSet.getDouble(15));        }else{
                System.out.println("nessun Stipendio caricato");
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);    }
    ...
    public static String isInAstensione(LocalDate dataCorrente,int matricola) {
        try {
            String sql = "select categoria from richiesta where ? >= Richiesta.data_inizio and ?<=Richiesta.data_fine and ref_impiegato = ?";
            preparedStatement = conn.prepareStatement(sql);
    ...
    public static boolean verifyPasswordA(String password, int matricola)  {
        resultSet=null;
        try{
            String sql = "SELECT password FROM Amministrativo WHERE password=? AND matricola=?";
            if(resultSet.next()) {
    ...
    public static void updateImpiegatoDisattivato(int matricolaImpiegato){
        try {
            String sql = "UPDATE Impiegato SET disattivato = 1 WHERE matricola =?";
            preparedStatement = conn.prepareStatement(sql);
            preparedStatement.setInt(1, matricolaImpiegato);
            preparedStatement.execute();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
}
    public static List<Object> deletePropostaTurni() {
        List<Object> ritorno = new ArrayList<>();    try {
            Connection conn = DriverManager.getConnection(URL, username, passwordDBMS);        String deleteSQL = "DELETE FROM PropostaTurno ";
            PreparedStatement preparedStatement = conn.prepareStatement(deleteSQL);
    ...
    public static List<Integer> getMatricole() {
        ArrayList<Integer> listaRitorno = new ArrayList();        try {
            String sql = "select matricola from Impiegato ";
            preparedStatement = conn.prepareStatement(sql);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {                listaRitorno.add(resultSet.getInt("matricola"));
            }
            return listaRitorno;        } catch (SQLException ex) {
            throw new RuntimeException(ex);        }
    }
    public static void insertTurni(List<PropostaTurno> proposta) {
        try {        String sql = "INSERT INTO Turno (ref_impiegato, tipo_turno, inizio_turno, fine_turno, data_turno)values ( ?,?, ?, ?, ?) ";
            preparedStatement = conn.prepareStatement(sql);
            for (PropostaTurno turno : proposta) {
                preparedStatement.setInt(1, PropostaTurno.getRefImpiegato());
                preparedStatement.setString(2, PropostaTurno.getTurno());            preparedStatement.setTime(3, Time.valueOf(PropostaTurno.getInizioTurno()));
                preparedStatement.setTime(4, Time.valueOf(PropostaTurno.getFineTurno()));            preparedStatement.setDate(5, Date.valueOf(PropostaTurno.getDataTurno()));
                preparedStatement.executeUpdate();        }
    ...
    public static List<Turno> getTurni(){
        ...
        //todo mettere dataTurno
        try {
            String sql = "Select tipo_turno,cognome, I.ref_servizio,I.ruolo,data_turno,nome,inizio_turno,fine_turno,ref_impiegato from Turno T,Impiegato I where T.ref_impiegato = I.matricola";
            PreparedStatement pstm = conn.prepareStatement(sql);
            resultSet = pstm.executeQuery();

            while (resultSet.next()) {
                //todo dovrebbe essere così: listaRitorno.add(new Turno(resultSet.getString(1), resultSet.getString(2) + " " +resultSet.getString(6), resultSet.getInt(3), resultSet.getString(4), resultSet.getDate(5).toLocalDate(),resultSet.getTime(7).toLocalTime(),resultSet.getTime(8).toLocalTime(),resultSet.getInt(9))); , ma devo modificare la classe turno
                listaRitorno.add(new Turno(resultSet.getString(1), resultSet.getString(2) + " " +resultSet.getString(6), resultSet.getInt(3), resultSet.getString(4), resultSet.getDate(5).toLocalDate()));
            }
            return listaRitorno;
    ...
    public static void updatePassword(String password, String mailPersonale){
        try {
            String sql = "update Impiegato set password = ? where mail_personale = ?";
            preparedStatement = conn.prepareStatement(sql);
            preparedStatement.setString(1,password);
            preparedStatement.setString(2,mailPersonale);
            preparedStatement.execute();
            String sql1 = "update Datore set password = ? where mail_personale = ?";
            preparedStatement = conn.prepareStatement(sql1);
            preparedStatement.setString(1,password);
            preparedStatement.setString(2,mailPersonale);
            preparedStatement.execute();
            String sql2 = "update Amministrativo set password = ? where mail_personale = ?";
            preparedStatement = conn.prepareStatement(sql2);
            preparedStatement.setString(1,password);
            preparedStatement.setString(2,mailPersonale);
            preparedStatement.execute();
